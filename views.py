...
def contact_Dean(request):
    return render(request, 'contact_Dean.html')
...
def Signup(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(id_number=id_number).Notexists():
            messages.error(request, 'id_number already not exists. Please log in.')
            return redirect('private_request')
...
def delete_private_diary_view(request, request_id):
    # جلب السجل بناءً على ID
    private_diary = get_object_or_404(PrivateDiary, id=request_id)

    if request.method == 'POST':
        # حذف السجل
        private_diary.delete()
        # إعادة التوجيه إلى صفحة المعلم
        return redirect('Teacherpage')  
...
# حذف سجل من DiaryGroup
def delete_group_diary_view(request, request_id):
    # جلب السجل بناءً على ID
    group_diary = get_object_or_404(DiaryGroup, id=request_id)
    ...
    # عرض صفحة تأكيد الحذف
    return render(request, 'delete_group_diary.html', {'group_diary':group_diary})
